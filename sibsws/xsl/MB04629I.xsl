<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0" xmlns:util="http://sibs.dsp.util">
	...
	<xsl:template match="MSG">
		...
		<xsl:value-of select="util:encodePack(string(format-number(number(if (count(payeeSequenceNumber)) then payeeSequenceNumber else 0), '000000')), xs:integer(number(6)),xs:integer(number(0))) " />
		<xsl:value-of select="util:encodePack(string(format-number(number(if (count(paymentSequenceNo)) then paymentSequenceNo else 0), '000000')), xs:integer(number(6)),xs:integer(number(0))) " />
	</xsl:template> 
